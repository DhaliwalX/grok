...
#define NextInstruction() (text_.at((size_t)instruction_register_.value_++))
...
bool handle_call(Machine *machine, Bytecode &bytecode) {
  ...
  if (function->IsNative()) {
    ...
    if (mac.returned_ == true && !mac.stack_.empty()) {
...
bool do_ret(Machine *machine, Bytecode &bytecode,
            std::shared_ptr<JSFunction> function) {
  return false;
}
...
std::pair<unsigned long long, bool> Machine::effective_address(Bytecode &code) {
...
void Machine::decode_registers(int reg, int &a, int &b, int &c) {
  a = reg & 0x7;
  reg >>= 3;
  b = reg & 0x7;
  reg >>= 3;
...
J decode_type(Register r2, Register r3) {
  ...
  if (t1 == J::_number && t2 == J::_number)
    return t1;

  if ((t1 == J::_string && t2 == J::_number) ||
      (t1 == J::_number && t2 == J::_string) ||
      (t1 == J::_string && t2 == J::_string))
    return J::_string;

  if (t1 == J::_object || t2 == J::_object)
    return J::_object;

  if (t1 == J::_array || t2 == J::_array)
    return J::_array;
...
bool Machine::binary_add() {
  ...
  switch (type) {
  case J::_number:
    res.handle_->obj_ = std::dynamic_pointer_cast<JSBasicObject, JSNumber>(
        std::make_shared<JSNumber>(a.handle_->obj_->GetNumber() +
                                   b.handle_->obj_->GetNumber()));
    res.type_ = DataType::Object;
    break;

  case J::_string:
    res.handle_->obj_ = std::dynamic_pointer_cast<JSBasicObject, JSString>(
        std::make_shared<JSString>(a.handle_->obj_->ToString() +
                                   b.handle_->obj_->ToString()));
    res.type_ = DataType::Object;
    break;
...
bool Machine::name() {                                                    \
    ...
    case J::_string:                                                           \
      res.handle_->obj_ = std::dynamic_pointer_cast<JSBasicObject, JSNumber>(  \
          std::make_shared<JSNumber>(a.handle_->obj_->ToString()               \
                                         op b.handle_->obj_->ToString()));     \
      res.type_ = DataType::Object;                                            \
      break;                                                                   \
                                                                               \
    default:                                                                   \
...
void Machine::execute() {
  ...
  while (
      status && (instruction_register_.Value() != size_text) &&
      (static_cast<Instruction>(bytecode.instruction_) != Instruction::HLT)) {
    ...
    case Instruction::OPUSHARROBJ: {
      ...
      if (IsArray(real_parent_array)) {
        ...
        case J::_number:
          object = Cast(real_parent_array, JSArray, JSBasicObject)
                       ->At(Cast(temp.handle_->obj_, JSNumber, JSBasicObject)
                                ->GetNumber());
          o_stack_.push_back(object);
          break;
        ...
        case J::_number:
          object->obj_ =
              Cast(real_parent_array, JSString, JSBasicObject)
                  ->At(Cast(temp.handle_->obj_, JSNumber, JSBasicObject)
                           ->GetNumber());
          o_stack_.push_back(object);
          break;
    ...
    case Instruction::LDAARR:
      accumulator_ =
          NewHandle(Cast(MakeShared(JSArray), JSBasicObject, JSArray));
      Continue();

    case Instruction::LDAOBJ:
      accumulator_ =
          NewHandle(Cast(MakeShared(JSObject), JSBasicObject, JSObject));
      Continue();

    case Instruction::LDANUM:
      accumulator_ =
          NewHandle(Cast(MakeShared(JSNumber), JSBasicObject, JSNumber));
      Continue();

    case Instruction::LDASTR:
      accumulator_ =
          NewHandle(Cast(MakeShared(JSString), JSBasicObject, JSString));
      Continue();

    case Instruction::RESIZEARR:
      std::dynamic_pointer_cast<JSArray, JSBasicObject>(
          accumulator_.handle_->obj_)
          ->resize((size_t)bytecode.value_.value_);
      Continue();
    ...
    case Instruction::POPTOARR: {
      ...
      if (!stack_.empty())
        ...
        arr->obj_ = std::make_shared<JSBasicObject>();
    ...
    case Instruction::POPTOOBJ: {
      ...
      if (!stack_.empty())
        ...
        object->obj_ = std::make_shared<JSBasicObject>();
      Cast(accumulator_.handle_->obj_, JSObject, JSBasicObject)
